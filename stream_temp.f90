...
! Declared Variables
     ...
     &     'hours', Dlit)/=0 )   CALL read_error(3,'dlit')
      ...
      IF ( declvar( MODNAME, 't_gw', 'nsegment', Nsegment, 'real', &
     &     'groundwater temperature', &
     &     'degrees C', t_gw)/=0 )   CALL read_error(3,'t_gw')
      
      ALLOCATE(t_ss(Nsegment))
      IF ( declvar( MODNAME, 't_ss', 'nsegment', Nsegment, 'real', &
     &     'subsurface temperature', &
     &     'degrees C', t_ss)/=0 )   CALL read_error(3,'t_ss')
      
      ALLOCATE(T_ground(Nsegment))
      IF ( declvar( MODNAME, 't_ground', 'nsegment', Nsegment, 'real', &
     &     'ground temperature', &
     &     'degrees C', t_ground)/=0 )   CALL read_error(3,'t_ground')

...
! Declared Variables
      ALLOCATE ( Seg_width_flow(Nsegment) )
      CALL declvar_real( MODNAME, 'seg_width_flow', 'nsegment', Nsegment, 'real', &
     &     'Width of each segment, flow-dependent', &
     &     'meters', Seg_width_flow )
...
! debug
     ...
     &                    " qlat = ", qlat, " seg_tave_lat = ", seg_tave_lat(i), " lat_temp_adj = ", lat_temp_adj(i,Nowmonth)
             write(*,*) "   width = ", Seg_width_flow(i), Nowyear, Nowmonth, Nowday
             continue
             exit
          endif
...
!             Compute the daily mean water temperature
              ! In: t_o, qlat, seg_tave_lat(i), te, ak1, ak2, i, seg_width_flow, seg_length/1000 (km)
              Seg_tave_water(i) = twavg(fs, t_o, qlat, seg_tave_lat(i), te, ak1, ak2, seg_width_flow(i), seg_length(i)/1000.0)
...
!
      ...
      USE PRMS_STRMTEMP, ONLY: Azrh, Alte, Altw, Seg_daylight, Seg_width_flow, &
...
!         azss = azso
         hrss = hrso
         sti = 0.0
         Svi = (rprnvg(hrsr, hrrs, hrss, sino, coso, sin_d, cosod, sinod, Seg_id)) / (Seg_width_flow(Seg_id) * totsh)
...
!  SOLVE FOR SHADE INCREMENTS THIS SEGMENT
         ...
         Svi = ((rprnvg(hrsr, hrrh, hrss, sino, coso, sin_d, cosod, sinod, Seg_id)) / (Seg_width_flow(Seg_id)*totsh))
!
...
!
      USE PRMS_STRMTEMP, ONLY: Azrh, Vce, Vdemx, Vhe, Voe, Vcw, Vdwmx, Vhw, Vow, Seg_width_flow, &
     &    Vdemn, Vdwmn, HALF_PI
      USE PRMS_BASIN, ONLY: NEARZERO
...
!  DETERMINE AMOUNT OF STREAM WIDTH SHADED
               ...
               IF ( bs < 0.0 ) bs = 0.0
               IF ( bs > Seg_width_flow(Seg_id) ) bs = Seg_width_flow(Seg_id)
